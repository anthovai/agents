...
def write_report(path: Path, data: dict) -> None:
    ...
    add("## นำไปใช้")
    add("")
    add("1. ใส่ค่าใน `.env` แล้ว `docker compose up -d face-service`")
    add("2. ใส่ค่าเดียวกันที่ Moodle: Site administration → Plugins → Local plugins → KAISER Proctor")
    add("3. ผลตรวจเก่าไม่ถูกตีความใหม่ — ทุกแถวใน `local_kaiproctor_check` เก็บเกณฑ์ที่ใช้ตอนนั้นไว้แล้ว")
    add("")
