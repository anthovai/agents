...
Ported unchanged from the face-re prototype — the licence is clean and the
approach held up in testing. If no model file is present the service still
runs but reports liveness as not evaluated; callers must treat that as a
failure in production, never as a pass.
"""
